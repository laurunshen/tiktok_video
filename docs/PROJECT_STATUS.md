...
> 最后更新：2026-05-30 — 手指畸形从「零容忍」降级为「仅产品特写人工把关」（见 §1 / §7）。
> 2026-05-29 — before-after 模板模式已删除；新增「分步工作流」(WorkflowWizard) A→D；模特库 + 关键帧 + 并行出视频。
...
- 核心需求：
  ...
  2. **角色一致性**（同一人贯穿全程）
  3. **关键瑕疵把关**（字幕泄漏 / 户外风声 仍零容忍；**手指畸形已降级**——日常镜头不管，仅"手拿产品的特写"时发布前人工扫一眼，理由见 §7）
...
| **agentic_segments（LLM 智能分镜）** | 🎬 生成 tab 开关 | LLM 把脚本拆 N 段、每段精简 prompt；自动选模特定妆照 + 产品图预生成各段首帧 → 各段并行生成 → 快切拼接；模特库为空时回退尾帧链式兜底；可选参考音色音频统一各段口播音色 | ✅ 代码完成 |
...
Phase 6: Seedance 2 (kie.ai 'bytedance/seedance-2')
   ├─ single_pass：1 次出整条
   └─ agentic_segments：buildAgenticSegmentPlanLLM 出 N 段 → 预生成各段首帧 → 并行生成
       （统一模特定妆照 + 产品图锁定；异常时回退尾帧链式兜底）→ ffmpeg 快切拼接
   ↓
...
2. **手部畸形 ~70%**：接近 Seedance 物理上限。**已决定不作为硬指标**（见 §7 决策表）——仅产品特写镜头发布前人工把关，日常镜头不处理。
...
| 手指畸形从「零容忍」降级为「仅特写人工把关」 | TikTok 信息流节奏快，观众看脸/产品/氛围，不会数手指；手部畸形 ~70% 已接近 Seedance 物理上限，硬追零畸形需「批量出 N 条挑 1 条」，成本/收益不划算。仅当手是画面主角（抓产品特写）时畸形才致命（暴露 AI 假感、伤信任），这种镜头发布前人工扫一眼即可。省下的精力投入 hook 完播率 + 产品一致性更值。 |
...
| `backend/services/agentic-runner.js` | 🎬 生成 tab 的 agentic 编排：v2 关键帧并行生成、串行尾帧兜底、拼接上传、job 状态更新 |
...
| `backend/routes/generate.js` | single_pass 主流程 + agentic_segments 分流（具体 agentic 编排已下沉到 `agentic-runner.js`） |
